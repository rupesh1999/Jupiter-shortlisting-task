...
export default function Settings() {
  ...
  const handleNameChange = e => {
    ...
    if (e.target.value.length < 5) {
      setNameError({
        error: true,
        msg: "must be grater than 5 character"
      });
    }
  };
  ...
  const handleDobChange = e => {
    ...
    if (!validDob) {
      setDobError({
        error: true,
        msg: "DOB should be of format YYYY-MM-DD"
      });
    }
    if (validDob) {
      setDobError({
  ...
  const handleProChange = e => {
    ...
    if (!containsNum) {
      setProError({
        error: true,
        msg: "Profession must contain only alphabets"
      });
    }
    if (containsNum) {
      setProError({
  ...
  const handleDesignationChange = e => {
    ...
    if (!containsNum) {
      setDesignationError({
        error: true,
        msg: "Designation must contain only alphabets"
      });
    }
    if (containsNum) {
      setDesignationError({
